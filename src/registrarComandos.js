...
const archivosComandos = fs.readdirSync(rutaComandos)
...
for (const archivo of archivosComandos) {
    ...
    if (comando.guildOnly) {
        comandosServidor.push(comando.data.toJSON());
    } else {
        comandosGlobales.push(comando.data.toJSON());
    }
}
